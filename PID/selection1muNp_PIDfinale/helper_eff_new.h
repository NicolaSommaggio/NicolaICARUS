...
ofstream MyFile("before_real.txt");
ofstream MyFile2("after_break_simul.txt");

ofstream MyFile3("after_break_simul2.txt");
ofstream MyFile4("after_break_simul3.txt");
ofstream MyFile5("after_break_simul4.txt");
ofstream MyFileDEDX("dedx.txt");
...
const SpillCut kCRTNeutrino([](const caf::SRSpillProxy* spill){
  for(const auto& match: spill->crt_hits) {
    double min_time =-0.2; double max_time =1.6;

    //if(match.time > min_time && match.time < max_time && match.plane>29 && match.plane<35){return false;} //This is top only
...
std::vector<double> compute_chi2_corrdEdx(const caf::Proxy<caf::SRSlice>& islc,
                                  std::size_t ipfp,
                                  int plane)
{
    std::vector<double> dedx, rr;
    ...
    for (const auto& pt : calo) {
      dedx.push_back(ShiftedDedx(pt.dedx,phi,Detector::ICARUS,Mode::R,1.));
      //MyFileDEDX << pt.dedx - ShiftedDedx(pt.dedx,phi,Detector::ICARUS,Mode::Standard,0.) << std::endl; 
        rr.push_back(pt.rr);
    }
...
PFPID id_pfp_corrdEdx(const caf::Proxy<caf::SRSlice>& islc,
             int ipfp,
             double dist_cut)
{
    // =========================================================
    ...
    std::vector<double> chi2 = compute_chi2_corrdEdx(islc, ipfp, DEFAULT_PLANE);
    if (chi2.size() < 2) {return PFPID::Unknown;}
...
ParticleCounts count_particles_corrdEdx(const caf::Proxy<caf::SRSlice>& islc,
                                int muon_index,
                                int dist_cut)
{
    ParticleCounts counts;
    ...
    for (std::size_t ipfp = 0; ipfp < islc.reco.npfp; ++ipfp) {
        ...
        PFPID pid = static_cast<PFPID>(id_pfp_corrdEdx(islc, ipfp, dist_cut));
...
int find_muon_corrdEdx(const caf::Proxy<caf::SRSlice>& islc, double dist_mucut)
{
    // =========================================================
    // Muon selection strategy:
    //   → longest primary contained track
...
bool automatic_selection_1muNp_new_corrdEdx(const caf::SRSpillProxy* sr,
                              const caf::Proxy<caf::SRSlice>& islc,
                              int dist_cut,
                              int cut_baryc)
{
    ...
    */
    ...
    // CUT 4 — Exactly one reconstructed muon candidate
    // =========================================================
    const int muon_index = find_muon_corrdEdx(islc, dist_cut);
    if (muon_index < 0) {
        return false;
    }
    ...
    // CUT 5 — Topology: 1μ + Np, no pions, no showers
    // =========================================================
    ParticleCounts counts = count_particles_corrdEdx(islc, muon_index, dist_cut);
...
CutResults automatic_selection_1muNp_new_cutflow(const caf::SRSpillProxy* sr,
                                                 const caf::Proxy<caf::SRSlice>& islc,
                                                 int dist_cut,
                                                 int cut_baryc)
{
    ...
    // =========================================================
    if (!isInFV(islc.vertex.x, islc.vertex.y, islc.vertex.z)) {
        //cout << " FIDUCIAL FAIL " << std::endl; 
        return res;
    }
    //cout << " FIDUCIAL TRUE " << std::endl;
    ...
    if (!kCRTNeutrino(sr)) 
    {
        //cout << " CRTPMT FALSE " << std::endl;
        //return res;
    }
    //cout << " CRTPMT TRUE " << std::endl;
    ...
    double barycenter   = bar_flash(sr,islc);
    double barycenter_x = bar_flash_x(sr,islc);
    double bar_charge   = islc.charge_center.z;
    double delta        = fabs(barycenter - bar_charge);

    //if (barycenter_x*islc.vertex.x <= 0){return res;}
    //if (barycenter <= -10000)             {return res;}
    //if (delta <= 0 || delta >= 100)       {return res;}

    //if (barycenter_x*islc.vertex.x <= 0){/*cout << " BARYCENTER FALSE diff cryo " << std::endl;*/ return res;}
    //if (barycenter <= -10000)             {cout << " BARYCENTER FALSE bary<-1000 "  << std::endl;return res;}
    //if (delta <= 0 || delta >= 100)       {cout << " BARYCENTER FALSE " << delta << std::endl;return res;}

    //cout << " BARYCENTER TRUE delta " << delta << std::endl;
    ...
    // =========================================================
    if (!all_contained(islc)) {
        //cout << " CONTAINMENT FALSE " << std::endl; 
        return res;
    }
    
    //cout << " CONTAINMENT TRUE " << std::endl;
    ...
    const int muon_index = find_muon(islc, dist_cut);
    if (muon_index < 0) {
        //cout << " NO MUON FALSE " << std::endl; 
        return res;
    }
       //cout << " FIND MUON TRUE " << std::endl;
    ...
    if (counts.pions != 0) {
       //cout << " NO PION FALSE " << std::endl; 
       return res;
    }
        //cout << " NO PION TRUE " << std::endl;
...
int MaxCutPassed(const CutResults& r)
{
    ...
    if (!r.crtpmt)      return 3;
    if (!r.barycenter)  return 4;
    if (!r.containment) return 5;
    //if (!r.crtpmt)      return 5;
    if (!r.muonID)      return 6;
    if (!r.protonID)    return 7;
    if (!r.pionID)      return 8;
    if (!r.showerID)    return 9;
    return 10; // passed all cuts
}
 ...
 double GetTrackMomentum(double trkrange, int pdg) 
  {
    ...
    if (trkrange < 0 || std::isnan(trkrange)) {
      std::cout << "Invalid track range " << trkrange << " return -1";
      return -1.;
    }
...
double T3D_angle_mup ( const caf::Proxy<caf::SRSlice>& islc, int ipfp_mu, double dist_cut ) {

double max_l=0;int index_p=-1;

    for (std::size_t ipfp = 0; ipfp < islc.reco.npfp; ++ipfp) {

        if (static_cast<int>(ipfp) == ipfp_mu) {
            continue;
        }
    ...
    int ipfp_pro=index_p;

    p_mu_x=(islc.reco.pfp[ipfp_mu].trk.rangeP.p_muon)*islc.reco.pfp[ipfp_mu].trk.dir.x;
...
const SpillMultiVar kReco_energy([](const caf::SRSpillProxy* sr)-> std::vector<double>
{
    ...
    for (auto const& islc : sr->slc){
        int ipfp_mu = -1;
	float amp=-2;float ene_nu=-1;float mu_end_x=-1;float mu_end_z=-1;float mu_start_x=-1;float mu_start_z=-1;float vtxx=-1;float vtxz=-1;float lmu=-1;

    if(automatic_selection_1muNp_new(sr, islc,10,100 )){
        ipfp_mu = find_muon(islc, 10);
        if(ipfp_mu!=-1){
	  ene_nu=Neutrino_energy_reco_Np(islc,ipfp_mu,10);
          amp=T3D_angle_mup(islc,ipfp_mu,10);
	  vector_active.push_back(ene_nu);
	  mu_end_x=islc.reco.pfp[ipfp_mu].trk.end.x;
	  mu_end_z=islc.reco.pfp[ipfp_mu].trk.end.z;
	  mu_start_x=islc.reco.pfp[ipfp_mu].trk.start.x;
	  mu_start_z=islc.reco.pfp[ipfp_mu].trk.start.z;
	  vtxx=islc.vertex.x;
	  vtxz=islc.vertex.z;
	  lmu=islc.reco.pfp[ipfp_mu].trk.len;
	  MyFile<<ene_nu<<" "<<mu_end_x<<" "<<mu_end_z<<" "<<mu_start_x<<" "<<mu_start_z<<" "<<vtxx<<" "<<vtxz<<" "<<lmu<<" "<<amp<<std::endl;
	}
...
const SpillMultiVar kReco_energy_post_break([](const caf::SRSpillProxy* sr)-> std::vector<double>
{
    std::vector<double> vector_active;

    for (auto const& islc : sr->slc){
        int ipfp_mu = -1;
	float ene_nu=-1;float mu_end_x=-1;float mu_end_z=-1;float mu_start_x=-1;float mu_start_z=-1;float vtxx=-1;float vtxz=-1;float lmu=-1;float amp=-2;
    ...
    if(automatic_selection_1muNp_new(sr, islc,10,100 )){
	  ...
	  vtxz=islc.vertex.z;
	  lmu=islc.reco.pfp[ipfp_mu].trk.len;
          if(mu_end_z<0. && mu_start_z<0.)
	    {
	      MyFile2<<ene_nu<<" "<<mu_end_x<<" "<<mu_end_z<<" "<<mu_start_x<<" "<<mu_start_z<<" "<<vtxx<<" "<<vtxz<<" "<<lmu<<" "<<amp<<std::endl;
	      vector_active.push_back(ene_nu);
	    }
          else if(mu_end_z>0. && mu_start_z>0.)
	    {
	      MyFile2<<ene_nu<<" "<<mu_end_x<<" "<<mu_end_z<<" "<<mu_start_x<<" "<<mu_start_z<<" "<<vtxx<<" "<<vtxz<<" "<<lmu<<" "<<amp<<std::endl;
	      vector_active.push_back(ene_nu);
	    }
		...
		{
		  float new_l_mu=lmu*fabs(mu_start_z)/fabs(mu_end_z-mu_start_z);
		  if(new_l_mu>50.)
		    {
		      mu_end_x=-999;
		      mu_end_z=0.;
		      ...
		      TVector3 p_mu_before(islc.reco.pfp[ipfp_mu].trk.dir.x * islc.reco.pfp[ipfp_mu].trk.rangeP.p_muon,
				    islc.reco.pfp[ipfp_mu].trk.dir.y * islc.reco.pfp[ipfp_mu].trk.rangeP.p_muon,
				    islc.reco.pfp[ipfp_mu].trk.dir.z * islc.reco.pfp[ipfp_mu].trk.rangeP.p_muon);   // GeV
		      
		      const double p_mu_mag_before = p_mu_before.Mag();                    // GeV
		      const double E_mu_before = std::sqrt((p_mu_mag_before * 1000.0)*(p_mu_mag_before * 1000.0)
						    + MUON_MASS*MUON_MASS)/1000.; // GeV
		      float mom_new_mu=GetTrackMomentum(new_l_mu, 13);
		      float p_mu_x_a=(mom_new_mu)*islc.reco.pfp[ipfp_mu].trk.dir.x; //GeV
		      float p_mu_y_a=(mom_new_mu)*islc.reco.pfp[ipfp_mu].trk.dir.y;
		      float p_mu_z_a=(mom_new_mu)*islc.reco.pfp[ipfp_mu].trk.dir.z;
		      float p_mu_tot_a = sqrt(p_mu_x_a*p_mu_x_a+p_mu_y_a*p_mu_y_a+p_mu_z_a*p_mu_z_a);             //GeV
		      float E_mu_a=sqrt(p_mu_tot_a*p_mu_tot_a+(105.658*105.658)/(1000*1000));
		      ene_nu=Neutrino_energy_reco_Np(islc,ipfp_mu,10)-E_mu_before+E_mu_a;
		      MyFile2<<ene_nu<<" "<<mu_end_x<<" "<<mu_end_z<<" "<<mu_start_x<<" "<<mu_start_z<<" "<<vtxx<<" "<<vtxz<<" "<<lmu<<" "<<amp<<std::endl;
		      vector_active.push_back(ene_nu);
...
const SpillMultiVar kReco_energy_post_break2([](const caf::SRSpillProxy* sr)-> std::vector<double>
{
    std::vector<double> vector_active;
    ...
    if(automatic_selection_1muNp_new(sr, islc,10,100 )){
        ...
        if(ipfp_mu!=-1){
          ...
          if(mu_end_z<0. && mu_start_z<0.)
            {
              MyFile3<<ene_nu<<" "<<mu_end_x<<" "<<mu_end_z<<" "<<mu_start_x<<" "<<mu_start_z<<" "<<vtxx<<" "<<vtxz<<" "<<lmu<<" "<<amp<<std::endl;
              vector_active.push_back(ene_nu);
            }
          else if(mu_end_z>0. && mu_start_z>0.)
            {
...
const SpillMultiVar kReco_energy_post_break3([](const caf::SRSpillProxy* sr)-> std::vector<double>
{
    std::vector<double> vector_active;
    ...
    if(automatic_selection_1muNp_new(sr, islc,10,100 )){
        ...
        if(ipfp_mu!=-1){
          ...
          if(mu_end_z<0. && mu_start_z<0.)
            {
              MyFile4<<ene_nu<<" "<<mu_end_x<<" "<<mu_end_z<<" "<<mu_start_x<<" "<<mu_start_z<<" "<<vtxx<<" "<<vtxz<<" "<<lmu<<" "<<amp<<std::endl;
              vector_active.push_back(ene_nu);
            }
          else if(mu_end_z>0. && mu_start_z>0.)
            {
...
const SpillMultiVar kReco_energy_post_break4([](const caf::SRSpillProxy* sr)-> std::vector<double>
{
    std::vector<double> vector_active;
    ...
    if(automatic_selection_1muNp_new(sr, islc,10,100 )){
        ...
        if(ipfp_mu!=-1){
          ...
          if(mu_end_z<0. && mu_start_z<0.)
            {
              MyFile5<<ene_nu<<" "<<mu_end_x<<" "<<mu_end_z<<" "<<mu_start_x<<" "<<mu_start_z<<" "<<vtxx<<" "<<vtxz<<" "<<lmu<<" "<<amp<<std::endl;
              vector_active.push_back(ene_nu);
            }
          else if(mu_end_z>0. && mu_start_z>0.)
            {
...
const SpillMultiVar kReco_energy_post_dEdx([](const caf::SRSpillProxy* sr)-> std::vector<double>
{
    std::vector<double> vector_active;
    ...
    for (auto const& islc : sr->slc){
      ...
      if(automatic_selection_1muNp_new_corrdEdx(sr, islc,10,100 )){
        ipfp_mu = find_muon_corrdEdx(islc, 10);
        if(ipfp_mu!=-1){
	  ene_nu=Neutrino_energy_reco_Np(islc,ipfp_mu,10);
	  vector_active.push_back(ene_nu);
	}
...
const SpillMultiVar kTrue_Enu_allnumuCC([](const caf::SRSpillProxy* sr)-> std::vector<double>
{
    ...
    if (!(std::isnan(nu.position.x) || std::isnan(nu.position.y) || std::isnan(nu.position.z))) {
        if (std::abs(nu.pdg) == 14  && nu.iscc && isInFV(nu.position.x, nu.position.y, nu.position.z)){
                vector_active.push_back(nu.E); 
        }
    }
    }
...
const SpillMultiVar kCount_entries([](const caf::SRSpillProxy* sr)-> std::vector<double>
{
    std::vector<double> vector_active;

    for (auto const& islc : sr->slc){

    if ((!std::isnan(islc.vertex.x) ||std::isnan(islc.vertex.y) ||std::isnan(islc.vertex.z) ||std::isnan(islc.charge_center.z))) {
        vector_active.push_back(1);
        if (isInFV(islc.vertex.x, islc.vertex.y, islc.vertex.z)){
            vector_active.push_back(2);
            if (kCRTNeutrino(sr)){
                 vector_active.push_back(2.5);
            ...
            if(barycenter_x*islc.vertex.x>0){
                vector_active.push_back(3);
                if (all_contained(islc)) {
                    vector_active.push_back(4);
                    if (kCRTNeutrino(sr)){
                        vector_active.push_back(5);
                        const int muon_index = find_muon(islc, 10);
                        if (!(muon_index < 0)){
                            vector_active.push_back(6);
                            ParticleCounts counts = count_particles(islc, muon_index, 10);
                            if(counts.protons > 0){
                                vector_active.push_back(7);
                                if(counts.pions == 0){
                                    vector_active.push_back(8);
                                    if(counts.showers == 0){vector_active.push_back(9);}
                                }
                            }
...
const SpillMultiVar kMuon_chi2mu_corrdEdx([](const caf::SRSpillProxy* sr)-> std::vector<double>
{
    std::vector<double> vector_active;
    ...
    if(automatic_selection_1muNp_new_corrdEdx(sr, islc,10,100 )){
        ipfp_mu = find_muon_corrdEdx(islc, 10);
        if(ipfp_mu!=-1){
    std::vector<double> chi2 = compute_chi2_corrdEdx(islc, ipfp_mu, DEFAULT_PLANE);
        vector_active.push_back(chi2[static_cast<int>(Chi2PID::Muon)]);
         }
...
const SpillMultiVar kMuon_chi2pro_corrdEdx([](const caf::SRSpillProxy* sr)-> std::vector<double>
{
    std::vector<double> vector_active;
