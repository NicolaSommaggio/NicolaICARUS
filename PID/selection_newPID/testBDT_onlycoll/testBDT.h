...
//BDT
std::string path_BDT_model = "GBDT_small_stat_newTS_newvars_onlycoll.txt";
#include "GBDT_model_inport.C"
...
//ofstream dump_purity("dump_purity.txt");
...
const SpillMultiVar testBDT([](const caf::SRSpillProxy* sr)-> std::vector<double>
{
    ...
    for(const auto &islc : sr->slc)
    {
        ...
        for (std::size_t ipfp(0); ipfp < islc.reco.npfp; ++ipfp)
        {
            ...
            //dump_purity << islc.reco.pfp[ipfp].trk.truth.p.pdg << " " << islc.reco.pfp[ipfp].trk.len << " " << Ek << " " << prediction << " " << islc.reco.pfp[ipfp].trackScore << " " << islc.reco.pfp[ipfp].parent_is_primary << " " << start_distance << " " << end_distance << " " << isInContained(islc.reco.pfp[ipfp].trk.end.x,islc.reco.pfp[ipfp].trk.end.y,islc.reco.pfp[ipfp].trk.end.z,5) << " " << islc.reco.pfp[ipfp].trk.truth.bestmatch.hit_completeness << " " << islc.reco.pfp[ipfp].trk.truth.bestmatch.hit_purity << endl;

            /*
            if(std::abs(islc.reco.pfp[ipfp].trk.truth.p.pdg) == 13)
            {
              if((prediction == 0 || prediction == 1) && islc.reco.pfp[ipfp].trk.len>2 && islc.reco.pfp[ipfp].trk.len<6)
              {
                //cout << "new track | depE: " << depE << endl;
                //for(int i=0; i<(int)lr.size(); i++){cout << lr[i] << " ";}
                //cout << endl;
                //for(std::size_t ihit(0); ihit < islc.reco.pfp[ipfp].trk.calo[bestplane].points.size(); ++ihit)
                //{
                //  cout << islc.reco.pfp[ipfp].trk.calo[bestplane].points[ihit].rr << " " << islc.reco.pfp[ipfp].trk.calo[bestplane].points[ihit].dedx << endl;
                //} 

                if(islc.reco.pfp[ipfp].trk.calo[2].points.size()>0)
                {
                  double start_point_x = islc.reco.pfp[ipfp].trk.calo[2].points[0].x;
                  double start_point_y = islc.reco.pfp[ipfp].trk.calo[2].points[0].y;
                  double start_point_z = islc.reco.pfp[ipfp].trk.calo[2].points[0].z;
                
                  double end_point_x = islc.reco.pfp[ipfp].trk.calo[2].points[islc.reco.pfp[ipfp].trk.calo[2].points.size()-1].x;
                  double end_point_y = islc.reco.pfp[ipfp].trk.calo[2].points[islc.reco.pfp[ipfp].trk.calo[2].points.size()-1].y;
                  double end_point_z = islc.reco.pfp[ipfp].trk.calo[2].points[islc.reco.pfp[ipfp].trk.calo[2].points.size()-1].z;

                  cout << sr->hdr.run << " " << sr->hdr.evt << " " << nslice << " vertex( " << islc.vertex.x << " " << islc.vertex.y << " " << islc.vertex.z << " ) start( " << start_point_x << " " << start_point_y << " " << start_point_z << " ) end( " << end_point_x << " " << end_point_y << " " << end_point_z << " )" << endl;
                }
              }

              //if(islc.reco.pfp[ipfp].trk.len < 20)continue;

              //dump_mu_length << islc.reco.pfp[ipfp].trk.len << endl;    

              //if(prediction == 0 || prediction == 1){dump_selected_mu_length << islc.reco.pfp[ipfp].trk.len << " " << islc.reco.pfp[ipfp].trackScore << " " << islc.reco.pfp[ipfp].parent_is_primary << " " << start_distance << " " << end_distance << " " << isInContained(islc.reco.pfp[ipfp].trk.end.x,islc.reco.pfp[ipfp].trk.end.y,islc.reco.pfp[ipfp].trk.end.z,5) << endl;}
            }
            if(std::abs(islc.reco.pfp[ipfp].trk.truth.p.pdg) == 2212)
            {
              //dump_pro_length << islc.reco.pfp[ipfp].trk.len << " " << Ek_proton << endl;    

              //if(prediction == 2 || prediction == 3){dump_selected_pro_length << islc.reco.pfp[ipfp].trk.len << " " << Ek_proton << " " << islc.reco.pfp[ipfp].trackScore << " " << islc.reco.pfp[ipfp].parent_is_primary << " " << start_distance << " " << end_distance << " " << isInContained(islc.reco.pfp[ipfp].trk.end.x,islc.reco.pfp[ipfp].trk.end.y,islc.reco.pfp[ipfp].trk.end.z,5) << endl;}
            }
            if(std::abs(islc.reco.pfp[ipfp].trk.truth.p.pdg) == 211)
            {
              //dump_pi_length << islc.reco.pfp[ipfp].trk.len << " " << Ek_pion << endl;    

              //if(prediction == 4 || prediction == 5){dump_selected_pi_length << islc.reco.pfp[ipfp].trk.len << " " << Ek_pion << " " << islc.reco.pfp[ipfp].trackScore << " " << islc.reco.pfp[ipfp].parent_is_primary << " " << start_distance << " " << end_distance << " " << isInContained(islc.reco.pfp[ipfp].trk.end.x,islc.reco.pfp[ipfp].trk.end.y,islc.reco.pfp[ipfp].trk.end.z,5) << endl;}
            }
          */
        }//loop over all pfps   
