...
TFile * f_prob_densities = TFile::Open("/exp/icarus/data/users/nsommagg/HISTO_prob_densities_6_classes_30percent_stat.root", "READ");
...
ofstream dumpNuE("/exp/icarus/data/users/nsommagg/dumpNuE_2_di_2_70percent.txt");

//std::array<int,20> interesting_runs = {9583,9342,9534,9944,9976,9926,9359,10085,9344,10065,9389,9735,9329,9342,9914,9337,9386,9409,9744,9807};
//std::array<int,20> interesting_evts = {8744,104641,23751,1395,45400,15618,15352,44128,34812,16661,17546,2178,24293,16189,65580,5408,4685,20228,3388,7351};
...
const SpillMultiVar DataLoader([](const caf::SRSpillProxy* sr)-> std::vector<double>
{
  ...
  for ( auto const& nu : sr->mc.nu )
  {
    ...
    /*
    for(int r = 0; r<20; r++){
    if(int(sr->hdr.run) == interesting_runs[r] && int(sr->hdr.evt) == interesting_evts[r])
    {
      cout << sr->hdr.run << " " << sr->hdr.evt << " " << nu.E << endl;
      cout << nu.position.x << " " << nu.position.y << " " << nu.position.z << endl;
      cout << std::abs(nu.pdg) << " " << nu.iscc << endl;
      cout << isInActive(nu.position.x,nu.position.y,nu.position.z) << " " << isInFV(nu.position.x,nu.position.y,nu.position.z) << " " << all_contained_mc(sr, nu) << endl;
      ...
      }//loop over all primary particles in the spill
      cout << n_muons << " " << muon_length << " " << n_protons_above << endl;
      cout << classification_type_MC(sr,nu) << endl;
    }
    }
    */
  }
