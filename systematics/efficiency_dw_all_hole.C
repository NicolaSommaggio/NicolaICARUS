...
//		//
	...
	ofstream oFile("histograms_all_muons/low_eff_traces.txt");
        ofstream outfiletracks("tracks_west_9435_t0.txt");
        ofstream outfiletracksE("tracks_east_9435_t0.txt");
...
//      	// 
	...
	if(file_number > line_count) {
		//warning message
		cout << endl << "### In " << file_list << " there are only " 
			<< line_count << " files ###" << endl << endl;	
...
//      	//        	
        	...
        	TTreeReaderValue<float>	lengthTPCW(myReaderTPCW, "length"), t0TPCW(myReaderTPCW, "t0PFP"),
        	...
        	TTreeReaderArray<unsigned short> 	
        				tpcTPCW_0(myReaderTPCW, "hits0.h.tpc");
        				
        	TTreeReaderArray<bool>	hasSPTPCW_0(myReaderTPCW, "hits0.h.hasSP");
        				
        	TTreeReaderArray<unsigned short>	
        				wireTPCW_0(myReaderTPCW, "hits0.h.wire");     				
        	...
        	TTreeReaderArray<unsigned short> 	
        				tpcTPCW_1(myReaderTPCW, "hits1.h.tpc");
        				
        	TTreeReaderArray<bool>	hasSPTPCW_1(myReaderTPCW, "hits1.h.hasSP");
        				
        	TTreeReaderArray<unsigned short>	
        				wireTPCW_1(myReaderTPCW, "hits1.h.wire");     				
        	...
        	TTreeReaderArray<unsigned short> 	
        				tpcTPCW_2(myReaderTPCW, "hits2.h.tpc");
        				
        	TTreeReaderArray<bool>	hasSPTPCW_2(myReaderTPCW, "hits2.h.hasSP");
        				
        	TTreeReaderArray<unsigned short>	
        				wireTPCW_2(myReaderTPCW, "hits2.h.wire");     				
...
//      	//        	
        	...
        	TTreeReaderValue<float>	lengthTPCE(myReaderTPCE, "length"),t0TPCE(myReaderTPCE, "t0PFP"),
        	...
        	TTreeReaderArray<unsigned short> 	
        				tpcTPCE_0(myReaderTPCE, "hits0.h.tpc");
        				
        	TTreeReaderArray<bool>	hasSPTPCE_0(myReaderTPCE, "hits0.h.hasSP");
        				
        	TTreeReaderArray<unsigned short>	
        				wireTPCE_0(myReaderTPCE, "hits0.h.wire");     				
        	...
        	TTreeReaderArray<unsigned short> 
					tpcTPCE_1(myReaderTPCE, "hits1.h.tpc");
        				
        	TTreeReaderArray<bool>	hasSPTPCE_1(myReaderTPCE, "hits1.h.hasSP");
        				
        	TTreeReaderArray<unsigned short>	
        				wireTPCE_1(myReaderTPCE, "hits1.h.wire");     				
        	...
        	TTreeReaderArray<unsigned short> 
					tpcTPCE_2(myReaderTPCE, "hits2.h.tpc");
        				
        	TTreeReaderArray<bool>	hasSPTPCE_2(myReaderTPCE, "hits2.h.hasSP");
        				
        	TTreeReaderArray<unsigned short>	
        				wireTPCE_2(myReaderTPCE, "hits2.h.wire");     				
		...
		while(myReaderTPCW.Next()) {	
			...
			if(*lengthTPCW > 100 && (*whicht0TPCW)==0) {
				outfiletracks << (*runTPCW) << " " << (*evtTPCW) <<  " 1 " << (*startxTPCW) << " " << (*startyTPCW) << " " << (*startzTPCW) << " " << (*endxTPCW) << " " << (*endyTPCW) << " " << (*endzTPCW) << " " << (*lengthTPCW) << " " << (*t0TPCW) << endl;
				//
				//	PLANE IND1 WEST
				//
				...
				for(int j = 0; j < wireTPCW_0.GetSize(); ++j) {
					...
					if(dqdxTPCW_0[j] > 0 && pitchTPCW_0[j] < 4 &&
						pitchTPCW_0[j] > 0 && !isnan(pitchTPCW_0[j])) {
						if(tpcTPCW_0[j] == 0) {
...
*/				
				...
				if(count_0 > wire_threshold) {
					...
					for(int k = min_wire_0; k <= max_wire_0; ++k) {
                        wcount_0+=w_0_0_w[k];
                        if(w_0_0_w[k]>0 && wire_array_0_0[k] == 1 && buco>0) {++count_0; hind1buchi->Fill(holder_0,buco);if(buco>max_buco)max_buco=buco;buco=0;}
				...
				if(count_1 > wire_threshold) {
					...
					for(int k = min_wire_1; k <= max_wire_1; ++k) {
                        wcount_1+=w_0_1_w[k];
                        if(w_0_1_w[k]>0 && wire_array_0_1[k] == 1 && buco>0) {++count_1;hind1buchi->Fill(holder_1,buco);if(buco>max_buco)max_buco=buco;buco=0;}
				...
				if(count_2 > wire_threshold) {
					...
					for(int k = min_wire_2; k <= max_wire_2; ++k) {
                        wcount_2+=w_0_2_w[k];
					  if(w_0_2_w[k]>0 && wire_array_0_2[k] == 1 && buco>0) {++count_2;hind1buchi->Fill(holder_2,buco);if(buco>max_buco)max_buco=buco;buco=0;}
				...
				if(count_3 > wire_threshold) {
					...
					for(int k = min_wire_3; k <= max_wire_3; ++k) {
                        wcount_3+=w_0_3_w[k];
                        if(w_0_3_w[k]>0 && wire_array_0_3[k] == 1 && buco>0) {++count_3;hind1buchi->Fill(holder_3,buco);if(buco>max_buco)max_buco=buco;buco=0;}
				...
				for(int j = 0; j < wireTPCW_1.GetSize(); ++j) {
					//conditions to consider a valid hit, can change
					if(dqdxTPCW_1[j] > 0 && pitchTPCW_1[j] < 4 &&
						pitchTPCW_1[j] > 0 && !isnan(pitchTPCW_1[j])) {
						if(tpcTPCW_1[j] == 0) {
...
*/	
				...
				if(count_01 > wire_threshold) {
					...
					for(int k = min_wire_01; k <= max_wire_01; ++k) {
                        ...
                        if(w_1_01_w[k]>0 && wire_array_1_01[k] == 1 && buco>0) {++count_01;hind2buchi->Fill(holder_01,buco);if(buco>max_buco)max_buco=buco;buco=0;}
				...
				if(count_23 > wire_threshold) {
					...
					for(int k = min_wire_23; k <= max_wire_23; ++k) {
                        ...
                        if(w_1_23_w[k]>0 && wire_array_1_23[k] == 1 && buco>0) {++count_23;hind2buchi->Fill(holder_23,buco);if(buco>max_buco)max_buco=buco;buco=0;}
				...
				for(int j = 0; j < wireTPCW_2.GetSize(); ++j) {
					//conditions to consider a valid hit, can change
					if(dqdxTPCW_2[j] > 0 && pitchTPCW_2[j] < 4 &&
						pitchTPCW_2[j] > 0 && !isnan(pitchTPCW_2[j])) {
						if(tpcTPCW_2[j] == 0) {
...
if(wire_array_2_23[k] == 1)h2_23_w->Fill(k);
				...
				if(count_01 > wire_threshold) {
					...
					for(int k = min_wire_01; k <= max_wire_01; ++k) {
                        ...
                        if(w_2_01_w[k]>0 && wire_array_2_01[k] == 1 && buco>0) {++count_01;hcollbuchi->Fill(holder_01,buco);if(buco>max_buco)max_buco=buco;buco=0;}
				...
				if(count_23 > wire_threshold) {
					...
					for(int k = min_wire_23; k <= max_wire_23; ++k) {
					  ...
					  if(w_2_23_w[k]>0 && wire_array_2_23[k] == 1 && buco>0) {++count_23;hcollbuchi->Fill(holder_23,buco);if(buco>max_buco)max_buco=buco;buco=0;}
		...
		while(myReaderTPCE.Next()) {	
			...
			//muon length must be greater than 1 meter
			if(*lengthTPCE > 100  && (*whicht0TPCE)==0) {
                                outfiletracksE << (*runTPCE) << " " << (*evtTPCE) <<  " 0 " << (*startxTPCE) << " " << (*startyTPCE) << " " << (*startzTPCE) << " " << (*endxTPCE) << " " << (*endyTPCE) << " " << (*endzTPCE) << " " << (*lengthTPCE) << " " << (*t0TPCE) <<  endl;
				...
				for(int j = 0; j < wireTPCE_0.GetSize(); ++j) {
					...
					if(dqdxTPCE_0[j] > 0 && pitchTPCE_0[j] < 4 &&
						pitchTPCE_0[j] > 0 && !isnan(pitchTPCE_0[j])) {
						if(tpcTPCE_0[j] == 0) {
...
if(wire_array_0_3[k] == 1)h0_3_e->Fill(k);
				...
				if(count_0 > wire_threshold) {
					...
					for(int k = min_wire_0; k <= max_wire_0; ++k) {
                        wcount_0+=w_0_0_e[k];
                        if(w_0_0_e[k]>0 && wire_array_0_0[k] == 1 && buco>0) {++count_0;hind1buchi->Fill(holder_0,buco);if(buco>max_buco)max_buco=buco;buco=0;}
				...
				if(count_1 > wire_threshold) {
					...
					for(int k = min_wire_1; k <= max_wire_1; ++k) {
                        wcount_1+=w_0_1_e[k];
					  if(w_0_1_e[k]>0 && wire_array_0_1[k] == 1 && buco>0) {++count_1;hind1buchi->Fill(holder_1,buco);if(buco>max_buco)max_buco=buco;buco=0;}
				...
				if(count_2 > wire_threshold) {
					...
					for(int k = min_wire_2; k <= max_wire_2; ++k) {
                        wcount_2+=w_0_2_e[k];
					  if(w_0_2_e[k]>0 && wire_array_0_2[k] == 1 && buco>0) {++count_2;hind1buchi->Fill(holder_2,buco);if(buco>max_buco)max_buco=buco;buco=0;}
				...
				if(count_3 > wire_threshold) {
					...
					for(int k = min_wire_3; k <= max_wire_3; ++k) {
                        wcount_3+=w_0_3_e[k];
					  if(w_0_3_e[k]>0 && wire_array_0_3[k] == 1 && buco>0) {++count_3;hind1buchi->Fill(holder_3,buco);if(buco>max_buco)max_buco=buco;buco=0;}
				...
				for(int j = 0; j < wireTPCE_1.GetSize(); ++j) {
					//conditions to consider a valid hit, can change
					if(dqdxTPCE_1[j] > 0 && pitchTPCE_1[j] < 4 &&
						pitchTPCE_1[j] > 0 && !isnan(pitchTPCE_1[j])) {
						if(tpcTPCE_1[j] == 0) {
...
if(wire_array_1_23[k] == 1)h1_23_e->Fill(k);
				...
				if(count_01 > wire_threshold) {
					...
					for(int k = min_wire_01; k <= max_wire_01; ++k) {
					  ...
					  if(w_1_01_e[k]>0 && wire_array_1_01[k] == 1 && buco>0) {++count_01;hind2buchi->Fill(holder_01,buco);if(buco>max_buco)max_buco=buco;buco=0;}
				...
				if(count_23 > wire_threshold) {
					...
					for(int k = min_wire_23; k <= max_wire_23; ++k) {
					  ...
					  if(w_1_23_e[k]>0 && wire_array_1_23[k] == 1 && buco>0) {++count_23;hind2buchi->Fill(holder_23,buco);if(buco>max_buco)max_buco=buco;buco=0;}
				...
				for(int j = 0; j < wireTPCE_2.GetSize(); ++j) {
					//conditions to consider a valid hit, can change
					if(dqdxTPCE_2[j] > 0 && pitchTPCE_2[j] < 4 &&
						pitchTPCE_2[j] > 0 && !isnan(pitchTPCE_2[j])) {
						if(tpcTPCE_2[j] == 0) {
...
if(wire_array_2_23[k] == 1)h2_23_e->Fill(k);
				...
				if(count_01 > wire_threshold) {
					...
					for(int k = min_wire_01; k <= max_wire_01; ++k) {
					  ...
					  if(w_2_01_e[k]>0 && wire_array_2_01[k] == 1 && buco>0) {++count_01;hcollbuchi->Fill(holder_01,buco);if(buco>max_buco)max_buco=buco;buco=0;}
				...
				if(count_23 > wire_threshold) {
					...
					for(int k = min_wire_23; k <= max_wire_23; ++k) {
					  ...
					  if(w_2_23_e[k]>0 && wire_array_2_23[k] == 1 && buco>0) {++count_23;hcollbuchi->Fill(holder_23,buco);if(buco>max_buco)max_buco=buco;buco=0;}
...
//		// 
	...
	TFile *outoutfilefile=new TFile("all_histo.root","RECREATE");
        prof_eff_vs_pitch_0->Write();        prof_eff_vs_pitch_2->Write();
        prof_eff_vs_pitch_1->Write();        hind1buchi->Write();hind2buchi->Write();hcollbuchi->Write();
