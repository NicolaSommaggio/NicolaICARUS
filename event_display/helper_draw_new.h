...
std::vector<std::pair<double,double>> rollingMedian(const caf::Proxy<caf::SRPFP> * pfp_ptr)
{
    std::vector<double> dedx_temp;
    std::vector<double> rr_temp;
    std::vector<std::pair<double, double>> rm;
    for(std::size_t ihit(0); ihit < (*pfp_ptr).trk.calo[2].points.size(); ++ihit)
    {
        if((*pfp_ptr).trk.calo[2].points[ihit].rr<30)
        {
            dedx_temp.push_back((*pfp_ptr).trk.calo[2].points[ihit].dedx);
            rr_temp.push_back((*pfp_ptr).trk.calo[2].points[ihit].rr);
        }
    }
...
const SpillMultiVar disegna([](const caf::SRSpillProxy* sr)-> std::vector<double>
{
    ...
    for (auto const& true_particle: sr->true_particles)
    {
        ...
        if(ptr_matched_pfp!=nullptr && ptr_matched_slice!=nullptr && std::abs(true_particle.pdg)==13 && (*ptr_matched_pfp).trk.truth.p.pdg==true_particle.pdg){v_ipfp_mu.push_back(ptr_matched_pfp); v_slice_mu.push_back(ptr_matched_slice);}
    ...
    std::vector<const caf::Proxy<caf::SRSlice> *> all_slices_in_splill;

    all_slices_in_splill.insert(all_slices_in_splill.end(), v_slice_mu.begin(), v_slice_mu.end());
    all_slices_in_splill.insert(all_slices_in_splill.end(), v_slice_pro.begin(), v_slice_pro.end());
    all_slices_in_splill.insert(all_slices_in_splill.end(), v_slice_pi.begin(), v_slice_pi.end());

    std::sort(all_slices_in_splill.begin(), all_slices_in_splill.end());
    all_slices_in_splill.erase(std::unique(all_slices_in_splill.begin(), all_slices_in_splill.end()), all_slices_in_splill.end());

    for(auto const & islc_ptr : all_slices_in_splill)
    {
        const caf::Proxy<caf::SRSlice>& islc = *islc_ptr;
        if(sr->hdr.evt % 2 !=0)continue;

        std::vector<int> vector_ipfp_mu;
        std::vector<int> vector_ipfp_pro;
        std::vector<int> vector_ipfp_pi;
        slice_counter+=1;
        for(int imuon=0; imuon<int(v_slice_mu.size()); imuon++)
        {
            if(!(v_slice_mu[imuon]==islc_ptr))continue;
            //rolling median
            std::vector<std::pair<double,double>> rm={{-1,-1}};
            dumpRM.push_back(rollingMedian(v_ipfp_mu[imuon]));
            rm=rollingMedian(v_ipfp_mu[imuon]);
            ...
            for(std::size_t ihit(0); ihit < (*v_ipfp_mu[imuon]).trk.calo[2].points.size(); ++ihit) 
            {
                if((*v_ipfp_mu[imuon]).trk.calo[2].points[ihit].rr<30)
                {
                    dedx_temp.push_back((*v_ipfp_mu[imuon]).trk.calo[2].points[ihit].dedx);
                    rr_temp.push_back((*v_ipfp_mu[imuon]).trk.calo[2].points[ihit].rr);
                    x_temp.push_back((*v_ipfp_mu[imuon]).trk.calo[2].points[ihit].x);
                }
                if((*v_ipfp_mu[imuon]).trk.calo[2].points[ihit].rr<5.) 
                {
                    dummy.push_back((*v_ipfp_mu[imuon]).trk.calo[2].points[ihit].dedx);
                }
            }
        ...
        /*
        for(int iproton=0; iproton<int(v_slice_pro.size()); iproton++)
        {
            if(!(v_slice_pro[iproton]==islc_ptr))continue;
        }
        for(int ipion=0; ipion<int(v_slice_pi.size()); ipion++)
        {
            if(!(v_slice_pi[ipion]==islc_ptr))continue;
        }
        */

        //dump informations on the slide
        const caf::Proxy<caf::SRPFP> * max_length_pfp=nullptr;
        double max_length=-1;

        dumpInfo << "*** RUN " << sr->hdr.run << " *** EVT " << sr->hdr.evt << " ***" << endl;

        for(const auto &ipfp : islc.reco.pfp){if(ipfp.trk.len>max_length){max_length_pfp=&ipfp; max_length=ipfp.trk.len;}}

        int ipfp_iterations=-1;
        for(const auto &ipfp : islc.reco.pfp)
        {
            ipfp_iterations++;
            if(ipfp_iterations==0)continue;
            TVector3 RecoVtx;
            RecoVtx.SetXYZ(islc.vertex.x, islc.vertex.y, islc.vertex.z);
            TVector3 RecoStart;
            RecoStart.SetXYZ(ipfp.trk.start.x,ipfp.trk.start.y,ipfp.trk.start.z);
            TVector3 RecoEnd;
            RecoEnd.SetXYZ(ipfp.trk.end.x,ipfp.trk.end.y,ipfp.trk.end.z);
            ...
            dumpInfo << "start coordinates " << ipfp.trk.start.x << " " << ipfp.trk.start.y << " " << ipfp.trk.start.z << endl;
            dumpInfo << "end coordinates " << ipfp.trk.end.x << " " << ipfp.trk.end.y << " " << ipfp.trk.end.z << endl;
            dumpInfo << "collection ";
            for (std::size_t ihit(0); ihit < ipfp.trk.calo[2].points.size(); ++ihit ){dumpInfo << ipfp.trk.calo[2].points[ihit].x << " ";}
            dumpInfo << endl;
            dumpInfo << "induction 1 ";
            for (std::size_t ihit(0); ihit < ipfp.trk.calo[0].points.size(); ++ihit ){dumpInfo << ipfp.trk.calo[0].points[ihit].x << " ";}
            dumpInfo << endl;
            dumpInfo << "induction 2 ";
            for (std::size_t ihit(0); ihit < ipfp.trk.calo[1].points.size(); ++ihit ){dumpInfo << ipfp.trk.calo[1].points[ihit].x << " ";}
            dumpInfo << endl;
            dumpInfo << "trackscore " << ipfp.trackScore << endl;
            if(&ipfp==max_length_pfp)dumpInfo << "is the longest, with length " << ipfp.trk.len << endl;
            else dumpInfo << "track length " << ipfp.trk.len << " track distance " << (RecoEnd - RecoStart).Mag() << endl;
            dumpInfo << "vertex - start distance " << (RecoVtx-RecoStart).Mag() << " vertex - end distance " << (RecoVtx-RecoEnd).Mag() << endl;
            dumpInfo << "pdg code " << ipfp.trk.truth.p.pdg << endl;
            dumpInfo << "is contained " << isInContained(ipfp.trk.end.x, ipfp.trk.end.y, ipfp.trk.end.z, 5.0) << endl;
            dumpInfo << "is the same TPC as vertex " << ((ipfp.trk.end.x*islc.vertex.x)>0) << endl;
            dumpInfo << "parent is primary " << ipfp.parent_is_primary << endl;
            Start_mom_v_proton.SetXYZ((ipfp.trk.rangeP.p_proton)*ipfp.trk.dir.x,(ipfp.trk.rangeP.p_proton)*ipfp.trk.dir.y,(ipfp.trk.rangeP.p_proton)*ipfp.trk.dir.z);
            Start_mom_v_pion.SetXYZ((ipfp.trk.rangeP.p_pion)*ipfp.trk.dir.x,(ipfp.trk.rangeP.p_pion)*ipfp.trk.dir.y,(ipfp.trk.rangeP.p_pion)*ipfp.trk.dir.z);
            ...
            for(int imuon=0; imuon<int(v_slice_mu.size()); imuon++)
            {
                ...
                for(std::size_t ihit(0); ihit < (*v_ipfp_mu[imuon]).trk.calo[2].points.size(); ++ihit) 
                {
                    if((*v_ipfp_mu[imuon]).trk.calo[2].points[ihit].rr<5.) { dummy.push_back((*v_ipfp_mu[imuon]).trk.calo[2].points[ihit].dedx); }
                }
                if(dummy.size()!=0) median=mediana(dummy);
                dumpInfo << "median " << median << endl;
                std::vector<std::pair<double,double>> rm={{-1,-1}};
                rm=rollingMedian(v_ipfp_mu[imuon]);
                dumpInfo << "rolling median ";
                for(int i=0; i<int(rm.size()); i++) dumpInfo << rm[i].first << " ";
                dumpInfo << endl;
                for(int i=0; i<int(rm.size()); i++) dumpInfo << rm[i].second << " ";
                dumpInfo << endl;

                if(&ipfp==v_ipfp_mu[imuon]) 
                {
                    dumpInfo << "MUON" << endl << endl;
                    vector_ipfp_mu.push_back(ipfp_iterations);
                }
            }
                
            //ci sono un sacco di copie di protone - come mai ??
            for(int iproton=0; iproton<int(v_slice_pro.size()); iproton++)
            {
                if(!(v_slice_pro[iproton]==islc_ptr))continue;
                if(&ipfp==v_ipfp_pro[iproton]) 
                {
                    dumpInfo << "PROTON" << endl << endl;
                    vector_ipfp_pro.push_back(ipfp_iterations);
                }
            }
            for(int ipion=0; ipion<int(v_slice_pi.size()); ipion++)
            {
                if(!(v_slice_pi[ipion]==islc_ptr))continue;
                if(&ipfp==v_ipfp_pi[ipion]) 
                {
                    dumpInfo << "PION" << endl << endl;
                    vector_ipfp_pi.push_back(ipfp_iterations);
                }
            }
        ...
        SliceDrawInfo drawInfo = GetSliceDrawInfo(islc,vector_ipfp_mu,vector_ipfp_pro,vector_ipfp_pi,true_particles);
