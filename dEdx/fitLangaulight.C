...
FitOutput fitLangaulight_unbinned(TH1D* h, std::vector<double> v, double x_low, double x_high, TDirectory *fitPrintFile, std::string modelChoice , int nbin){
        ...
        //PASSING MUON CONTRIBUTION
        RooRealVar meanMIP("mean_mip", "Most Probable Value for passing", 1.8, 0.5, 2.5);
        RooRealVar sigmaMIP("sigmaMIP", " width for passing", 1., 0., 15.);
        RooGaussian curveMIP("curveMIP", "curve PDF for passing", x, meanMIP, sigmaMIP);
        ...
        else if (modelChoice== "dscbxgauss") 
        {
            chosenModel = &dscbxgauss;
        }
        ...
        */
        ...
        fitParameters.push_back(fitRes->minNll()); //7
        fitParameters.push_back(fitRes->status()); //8
        fitParameters.push_back(fitRes->covQual()); //9

        fitParameters.push_back(vec_larghezza[2]);  //10

        fitParameters.push_back(meanG.getVal()); //11
