const SpillMultiVar kMuon_endx_simul_MC_Np([](const caf::SRSpillProxy* sr)-> std::vector<double>
{
    ...
    float start_muon, end_muon,start_muon_t, end_muon_t, l_muon;
	...
	if(ipfp_mu!=-1){
	  ...
	  E_nu=(E_mu+E_p)/1000.;
	    ...
	    start_muon=fabs(islc.reco.pfp[ipfp_mu].trk.start.x);
	    end_muon=fabs(islc.reco.pfp[ipfp_mu].trk.end.x);
	    start_muon_t=(islc.reco.pfp[ipfp_mu].trk.start.x);
	    end_muon_t=(islc.reco.pfp[ipfp_mu].trk.end.x);
      l_muon=islc.reco.pfp[ipfp_mu].trk.len;
      

      if(start_muon<210.07 && end_muon<210.07) {vector_active.push_back(islc.reco.pfp[ipfp_mu].trk.end.x);MyFile<<E_nu<<" "<<E_nu_a<<" "<<p_T<<" "<<p_T_a<<" "<<end_muon_t<<" "<<end_muon_t<<" "<<start_muon_t<<" "<<l_muon<<" "<<l_muon<<" "<<p_mu_tot<<" "<<p_mu_tot<<" "<<mu_dirx<<" "<<mu_diry<<" "<<mu_dirz<<" "<<max_p_e<<" "<<max_p_l<<" "<<max_p_dirx<<" "<<max_p_diry<<" "<<max_p_dirz<<std::endl;}
      else if(start_muon>210.07 && end_muon>210.07) {vector_active.push_back(islc.reco.pfp[ipfp_mu].trk.end.x);MyFile<<E_nu<<" "<<E_nu_a<<" "<<p_T<<" "<<p_T_a<<" "<<end_muon_t<<" "<<end_muon_t<<" "<<start_muon_t<<" "<<l_muon<<" "<<l_muon<<" "<<p_mu_tot<<" "<<p_mu_tot<<" "<<mu_dirx<<" "<<mu_diry<<" "<<mu_dirz<<" "<<max_p_e<<" "<<max_p_l<<" "<<max_p_dirx<<" "<<max_p_diry<<" "<<max_p_dirz<<std::endl;}
      else{
	...
	if((random>1161.*0.33/6133. && islc.reco.pfp[ipfp_mu].trk.end.x<0.) || (random>1161.*0.11/6133. && islc.reco.pfp[ipfp_mu].trk.end.x>0.)){vector_active.push_back(islc.reco.pfp[ipfp_mu].trk.end.x);MyFile<<E_nu<<" "<<E_nu_a<<" "<<p_T<<" "<<p_T_a<<" "<<end_muon_t<<" "<<end_muon_t<<" "<<start_muon_t<<" "<<l_muon<<" "<<l_muon<<" "<<p_mu_tot<<" "<<p_mu_tot<<" "<<mu_dirx<<" "<<mu_diry<<" "<<mu_dirz<<" "<<max_p_e<<" "<<max_p_l<<" "<<max_p_dirx<<" "<<max_p_diry<<" "<<max_p_dirz<<std::endl;}
	else
	  {
	    float new_l_mu=l_muon*fabs(start_muon-210.07)/fabs(end_muon-start_muon);
	    if(new_l_mu>50.)
	      {
		vector_active.push_back(210.*islc.reco.pfp[ipfp_mu].trk.end.x/end_muon);
		...
		if(start_muon_t<0)MyFile<<E_nu<<" "<<E_nu_a<<" "<<p_T<<" "<<p_T_a<<" "<<end_muon_t<<" -210."<<" "<<start_muon_t<<" "<<l_muon<<" "<<new_l_mu<<" "<<p_mu_tot<<" "<<p_mu_tot_a<<" "<<mu_dirx<<" "<<mu_diry<<" "<<mu_dirz<<" "<<max_p_e<<" "<<max_p_l<<" "<<max_p_dirx<<" "<<max_p_diry<<" "<<max_p_dirz<<std::endl;
		if(start_muon_t>0)MyFile<<E_nu<<" "<<E_nu_a<<" "<<p_T<<" "<<p_T_a<<" "<<end_muon_t<<" 210."<<" "<<start_muon_t<<" "<<l_muon<<" "<<new_l_mu<<" "<<p_mu_tot<<" "<<p_mu_tot_a<<" "<<mu_dirx<<" "<<mu_diry<<" "<<mu_dirz<<" "<<max_p_e<<" "<<max_p_l<<" "<<max_p_dirx<<" "<<max_p_diry<<" "<<max_p_dirz<<std::endl;
	      }
	    else
	      {
		MyFile<<E_nu<<" -1. "<<p_T<<" -1. "<<end_muon_t<<" -1001."<<" "<<start_muon_t<<" "<<l_muon<<" -1."<<" "<<p_mu_tot<<" -1. "<<mu_dirx<<" "<<mu_diry<<" "<<mu_dirz<<" "<<max_p_e<<" "<<max_p_l<<" "<<max_p_dirx<<" "<<max_p_diry<<" "<<max_p_dirz<<std::endl;
	      }
	  }
      }
    }
