...
constexpr double MIN_MUON_LENGTH   = ; //GUMP

//constexpr double MAX_MUON_LENGHT = 400.0; //GUMP
constexpr double MAX_MUON_LENGHT = ; //GUMP
...
constexpr double PROTON_KE_MIN        = ; //GUMP
...
// Returns: 0 = east only, 1 = west only, 2 = both, -1 = neither
/*
int cryo_selection_from_light(const caf::SRSpillProxy* sr)
{
    constexpr double kMinTime = -0.6;
    constexpr double kMaxTime = 1.8;

    bool cryo_west = false;
    bool cryo_east = false;

    for (const auto& match : sr->crtpmt_matches)
    {
        if (match.flashGateTime <= kMinTime || match.flashGateTime >= kMaxTime)
            continue;

        if (match.flashPosition.x > 0)
            cryo_west = true;
        else if (match.flashPosition.x < 0)
            cryo_east = true;

        // Early exit: both sides already confirmed, no need to keep looping
        if (cryo_west && cryo_east)
            break;
    }

    if (cryo_west && cryo_east) return 2;
    if (cryo_west)              return 1;
    if (cryo_east)               return 0;
    return -1;
}
*/
...
int cryo_selection_from_light(const caf::SRSpillProxy* sr)
{
    constexpr double kMinTime = -0.6;
    constexpr double kMaxTime = 1.8;
    constexpr double PEthreshold = 3000./0.341;// valid for run4
    bool cryo_west = false;
    bool cryo_east = false;

    for (const auto& match : sr->crtpmt_matches)
    {
        if (match.flashGateTime <= kMinTime || match.flashGateTime >= kMaxTime || match.flashPE<PEthreshold)
            continue;
                              ...
                              int cut_baryc)
{
    ...
    int select_cryo=cryo_selection_from_light(sr);
    if(select_cryo==-1)return false;
    int cryo_sclice=-1;
    if(islc.vertex.x<0)cryo_sclice=0;
    if(islc.vertex.x>0)cryo_sclice=1;
     if(select_cryo<2)
      {
        if(cryo_sclice!=select_cryo)return false;
      }
...
*/
        ...
        int select_cryo=cryo_selection_from_light(sr);
    if(select_cryo==-1)return res;
    int cryo_sclice=-1;
    if(islc.vertex.x<0)cryo_sclice=0;
    if(islc.vertex.x>0)cryo_sclice=1;
     if(select_cryo<2)
      {
        if(cryo_sclice!=select_cryo)return res;
      }
...
const SpillMultiVar kReco_energy([](const caf::SRSpillProxy* sr)-> std::vector<double>
{
    ...
    if(automatic_selection_1muNp_new(sr, islc,10,100 )){
        ...
        if(ipfp_mu!=-1){vector_active.push_back(Neutrino_energy_reco_Np(islc,ipfp_mu,10)); }
...
double final_gate_delta = 0;
double final_livetime = 0;

const SpillVar gate_delta([](const caf::SRSpillProxy* spill)-> double
{ 

        final_gate_delta += spill->hdr.triggerinfo.gate_delta;
        //cout << "Delta " << spill->hdr.triggerinfo.gate_delta << endl;
        final_livetime += 1;
