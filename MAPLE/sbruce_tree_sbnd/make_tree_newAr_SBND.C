...
#include "/exp/icarus/app/users/marterop/dev_areas/sbn_syst/syst_macros/helper_sbnd_variables.h"
...
void make_tree_newAr_SBND(std::string outname = "/exp/sbnd/data/users/marterop/Ar23+_iterE/dent_nolight/SBND_MC_official_nolight_nocrosser_6.root")
...
//SBND DIRT -- 
  ...
  const SpillCut kSpillSelection = kPECut;
  const Cut kSliceSelection = Selection_1muNp_slice_sbnd;
  ...
  std::vector<std::string> nu_branch_names = {
    ...
    "Muon_chi2mu","Muon_chi2pro","Vertex_x","Vertex_y","Vertex_z","Proton_endx","Proton_endy","Proton_endz","Transverse_angle","T3D_angle_mup","DeltaZ",
    "Transverse_mom_reco_mu","Transverse_mom_reco_pro","Number_protons","Muon_trackScore","Proton_trackScore","Off_axis_angle_sbnd",
  };
  ...
  std::vector<Var> nu_vars = {
    ...
    kMuon_chi2mu,kMuon_chi2pro,kVertex_x,kVertex_y,kVertex_z,kProton_endx,kProton_endy,kProton_endz,kTransverse_angle,kT3D_angle_mup,kdeltaZ,
    kTransverse_mom_reco_mu,kTransverse_mom_reco_pro,kNumber_protons,kMuon_trackScore,kProton_trackScore,kOff_axis_angle_sbnd,
  };
